...
public:
  ...
  int execute(std::string const &content) override {
    ...
    if (result) {
      throw ParsingException(std::string("Error in execute: ") +
                             result.message());
    }
  ...
  void createFile(std::string const &file_name) {
    ...
    if (op_result) {
      throw ParsingException(std::string("Error in createFile: ") +
                             op_result.message());
    }
  ...
  std::vector<std::string> generateListOfFiles(std::string const &content) {
    ...
    std::cout << "generatlistoffiles: " << result.size() << std::endl;
    return result;
  }
